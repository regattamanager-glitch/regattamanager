...
import { User, Anchor, Users, Trash2, CreditCard, Info, ChevronRight, Phone, ShieldAlert, Award, ShoppingBag, Save } from "lucide-react";
...
export default function ManualRegistrationPage() {
  ...
  const [skipper, setSkipper] = useState<Skipper>({
    seglerId: "", // Bei manueller Anlage oft leer oder wird später verknüpft
    name: "",
    nation: "GER",
    email: "",
    verein: "",
  ...
  const calculateTotal = () => {
    ...
    // Bei Vereins-Manual-Entry entfällt oft die 8% Service-Fee, falls bar gezahlt wird.
    // Ich lasse sie hier zur Info stehen, aber man könnte sie auf 0 setzen.
    const fee = subtotal * 0.08; 
    return { subtotal, fee, total: subtotal + fee };
  };

  const handleSubmit = async () => {
    if (!confirm("Anmeldung manuell anlegen und direkt als 'Bezahlt' markieren?")) return;
    
    setSubmitting(true);
    const usedKlasse = decodeURIComponent(klasseFromUrl || "");

    try {
      // DIREKTES SPEICHERN in der Datenbank (Bypass Stripe)
      const response = await fetch("/api/registrations", {
        ...
        body: JSON.stringify({
          ...
          status: "PAID", // Direkt als bezahlt markieren bei manuellem Entry
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center">
      ...
      <p className="font-bold text-sky-400 uppercase tracking-tighter">Lade System...</p>
    </div>
  );
      ...
      <div className="max-w-5xl mx-auto pt-12 px-4">
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <p className="text-amber-200 text-sm font-bold uppercase tracking-tight">
              Manueller Admin-Eintrag für: <span className="text-white">{event.name}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <FormSection title="Skipper Details" icon={<User className="text-sky-400 w-5 h-5" />}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <InputGroup label="Sponsor" name="sponsor" icon={<Award className="w-3 h-3"/>} value={skipper.sponsor} onChange={handleSkipperChange} />
              </div>
            </FormSection>
            ...
            <section className="bg-[#1e293b] border border-slate-700 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
                <h2 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 text-white">
                   <Users className="text-sky-400 w-5 h-5" /> Crew Mitglieder
                </h2>
                <button onClick={() => setCrew([...crew, { 
                    name: "", nation: "GER", verein: "", geschlecht: "m", geburtsTag: "", geburtsMonat: "", geburtsJahr: "", lizenzNummer: "", email: "", telefon: ""
                  } as CrewMember])} 
              ...
              <div className="space-y-6">
                {crew.length === 0 && <p className="text-center text-slate-500 text-xs uppercase font-bold py-4">Keine Crew-Mitglieder eingetragen</p>}
                {crew.map((member, i) => (
                  <div key={i} className="p-5 bg-[#0f172a]/40 border border-slate-700 rounded-xl relative group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <InputGroup label="Name" value={member.name} onChange={(e: any) => { const c = [...crew]; c[i].name = e.target.value; setCrew(c); }} required />
                      <InputGroup label="Club" value={member.verein} onChange={(e: any) => { const c = [...crew]; c[i].verein = e.target.value; setCrew(c); }} />
                      <InputGroup label="Nation" value={member.nation} onChange={(e: any) => { const c = [...crew]; c[i].nation = e.target.value; setCrew(c); }} required />
                      <InputGroup label="Lizenz" value={member.lizenzNummer} onChange={(e: any) => { const c = [...crew]; c[i].lizenzNummer = e.target.value; setCrew(c); }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8 bg-sky-900/10 border border-sky-500/30 rounded-2xl p-6 backdrop-blur-md shadow-2xl">
              <h2 className="text-xl font-black mb-6 flex items-center gap-2 uppercase tracking-tighter text-white">
                <CreditCard className="text-sky-400 w-6 h-6" /> Übersicht
              </h2>
              <div className="space-y-3 mb-8">
                <PriceRow label="Meldegeld" value={costs.subtotal} />
                <PriceRow label="Service (System)" value={costs.fee} />
                <div className="h-px bg-slate-700 my-4" />
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black uppercase text-sky-400 tracking-widest">Gesamt</span>
